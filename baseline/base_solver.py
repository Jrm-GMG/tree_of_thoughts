...
class BaselineBaseSolver(ABC):
    ...
    def _run(
        ...
        try:
            responses = self.llm.generate(
                prompt,
                max_new_tokens=self.get_max_tokens(attempt),
    ...
    def get_max_tokens(self, attempt: int) -> int:
        """Maximum number of tokens to generate for an attempt.

        Args:
            attempt: Attempt index, starting at 0.

        Returns:
            The maximum token count for the given attempt.
        """
        return 300 if attempt < 2 else 500
