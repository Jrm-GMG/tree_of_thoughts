...
class ComparisonRunner(BaseRunner):
    ...
    def _run_tot(self, dataset: Union[pd.DataFrame, Dict[str, Any], None], num_problems: int, 
        ...
        for i in tqdm(range(num_problems), desc=progress_desc, unit='problem'):
            ...
            if self.config.get('verbose'):
                status = 'OK' if success else 'X'
                print(f"Problem {i+1}: {status} (nodes: {tree_stats['total_nodes']})")
    ...
    def _verify_math_solution(self, predicted: str, correct: str) -> bool:
        """Verify if math solution is correct"""
        if not predicted or not correct:
            return False
            
        # Use the task's verification method if available
        if hasattr(self.task, '_verify_mathematical_equivalence'):
            return self.task._verify_mathematical_equivalence(predicted, correct)
