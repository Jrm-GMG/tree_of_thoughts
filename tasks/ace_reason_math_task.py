...
from .prompts import get_ace_math_task_prompt, ACE_MATH_FORMAT_PROMPT
...
class AceReasonMathTask(Task):
    ...
    def parse_solution(self, node: TreeNode) -> Any:
        ...
        if answer and self.current_problem and self._verify_mathematical_equivalence(
            ...
            if self.debug:
                print(f"  Parsed solution: {answer}")
            return answer
    ...
    def get_random_problem_ids(self, n: int) -> List[int]:
        """
        Get n random problem IDs.
        
        Args:
            n: Number of problem IDs to return
            
        Returns:
            List of problem IDs
        """
        return random.sample(range(self.total_problems), min(n, self.total_problems))
    ...
    def get_dataset_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'domains': ['algebra', 'combinatorics', 'number_theory', 'geometry', 'calculus', 'probability'],
            'format_requirement': 'Final answers must be in \\boxed{} format',
            'average_problem_length': '~400 characters',
            'answer_format': 'Numerical or algebraic expressions'
        }